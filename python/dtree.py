...
def buildTree(dataset, attributes, maxdepth=1000000, min_samples_split=5, min_info_gain=0.01):
    "Recursively build a decision tree with pre-pruning"
    ...
    def buildBranch(dataset, default, attributes):
        ...
        return buildTree(dataset, attributes, maxdepth-1, min_samples_split, min_info_gain)

    default = mostCommon(dataset)
    
    # **Pre-pruning conditions**
    if maxdepth < 1 or len(dataset) < min_samples_split:
        return TreeLeaf(default)

    a = bestAttribute(dataset, attributes)
    
    # Compute the information gain of the best attribute
    info_gain = averageGain(dataset, a)
    
    # Stop splitting if information gain is too low (pre-pruning)
